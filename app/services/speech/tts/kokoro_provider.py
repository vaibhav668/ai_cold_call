...
_kokoro_executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="kokoro_tts")
...
class KokoroProvider(TextToSpeechProvider):
    ...
    async def warmup(cls) -> float:
        ...
        if model and model != "FAILED":
            ...
            # All production voices that could be requested in the demo (EN and HI)
            PRODUCTION_VOICES = [
                ...
                _settings.KOKORO_VOICE_ANANYA_HI,
            ]
            ...
            # Run dummy synthesis using primary English & Hindi production voices
            primary_voice_en = cls._style_cache.get(
                _settings.KOKORO_VOICE_SOPHIA_EN,
                _settings.KOKORO_VOICE_SOPHIA_EN
            )
            primary_voice_hi = cls._style_cache.get(
                _settings.KOKORO_VOICE_SOPHIA_HI,
                _settings.KOKORO_VOICE_SOPHIA_HI
            )
            try:
                logger.info("[WARMUP] Running dummy Kokoro synthesis on primary production voices (EN + HI)...")
                def _dummy_run():
                    model.create("Hello, warming up.", voice=primary_voice_en, speed=1.0, lang="en-us")
                    model.create("नमस्ते", voice=primary_voice_hi, speed=1.0, lang="hi")
                await asyncio.get_event_loop().run_in_executor(_kokoro_executor, _dummy_run)
                logger.info("[WARMUP] Dummy Kokoro synthesis completed (EN + HI).")
    ...
    def _resolve_voice_and_lang(self, voice_config: Optional[dict], language: Optional[str]) -> tuple[str, str]:
        ...
        default_voice = settings.KOKORO_VOICE_SOPHIA_HI if lang in ("hi", "hindi") else settings.KOKORO_VOICE_SOPHIA_EN
        voice = voice_map.get((persona, lang), default_voice)

        # Determine G2P language code for Kokoro phonemizer
        if lang in ("hi", "hindi"):
            lang_code = "hi"
            logger.info(f"[HINDI-TTS-CONFIG] persona='{persona}' language='{language}' → voice='{voice}' lang_code='hi'")
        else:
            lang_code = "en-us"
            logger.info(f"[KOKORO-VOICE] Resolved: persona='{persona}' lang='{lang}' → voice='{voice}' lang_code='{lang_code}'")

        # Explicit Guard: Fail loudly if Hindi is routed to English configuration
        if lang in ("hi", "hindi") and lang_code == "en-us":
            logger.error("[ERROR] Hindi TTS attempted with English language configuration!")
            raise ValueError("Hindi TTS configuration error: lang_code resolved to en-us for Hindi speech request")
    ...
    async def stream_speech(
        ...
        if lang_key in ("hi", "hindi"):
            # Native Hindi path: preserve Devanagari script completely
            processed_text = text.strip()
        elif lang_key in ("te", "telugu"):
            # Telugu path: transliterate Telugu script to Latin + ASCII filtering
            processed_text = transliterate_telugu(text)
            processed_text = "".join(c for c in processed_text if ord(c) < 128)
        else:
            # English path: standard English text
            processed_text = text.strip()
        ...
        if hasattr(model, "voices") and isinstance(model.voices, dict):
            if voice not in model.voices:
                fallback_voice = "hf_alpha" if lang_key in ("hi", "hindi") and "hf_alpha" in model.voices else ("af_sky" if "af_sky" in model.voices else list(model.voices.keys())[0])
        ...
        if lang_key in ("hi", "hindi"):
            logger.info(f"[HINDI-TTS] language=hi voice={voice} chars={len(processed_text)} text='{processed_text}'")
        else:
